...
const checkVendorOrderExists = async (orderNo, vendorId) => {
  try {
  ...
  return (
    <div key={vendor_id} className="mb-4 border rounded p-4 bg-gray-50">
      ...
      <ul>
        {products.map(p => (
          <li key={p.id} className="mb-3 flex items-start gap-4">
  <img
  ...
  <div className="flex flex-col">
    ...
    <div className="text-sm text-gray-600">Brand: {p.product.brand || 'N/A'}</div>
    <div className="text-sm text-gray-700">₹{p.product.price} × {p.quantity}</div>
  </div>
</li>
